...
public final class RSAUtils
{
      ...
      public static byte[] encryptData( byte[] data, PublicKey publicKey)
      {
             try
            {
                  Cipher cipher = Cipher.getInstance(KEY_ALGORITHM); //(RSA);//"RSA/ECB/PKCS1Padding"   就是：“算法/工作模式/填充模式”
                   // 编码前设定编码方式及密钥
      ...
      public static byte[] decryptData( byte[] encryptedData, PrivateKey privateKey)
      {
             try
            {
                  Cipher cipher = Cipher.getInstance(KEY_ALGORITHM); //Cipher.getInstance(RSA);
